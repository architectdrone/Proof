...
public class EnumConstantPattern extends UnparserPattern<JavaNode> {
    @Override
    public List<DisplayElement> unparse(final DisplayElement label, final DisplayElementAccessor<JavaNode> displayElementAccessor) {
        ...
        if (builder.getAccessor().contains(ARGUMENT_LIST))
        {
            builder.string("(");
            builder.node(ARGUMENT_LIST);
            builder.string(")");
        }
        if (builder.getAccessor().contains(TYPE_BODY)) {
            builder.space();
