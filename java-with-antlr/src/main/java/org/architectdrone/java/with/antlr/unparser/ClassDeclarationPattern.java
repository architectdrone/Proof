...
public class ClassDeclarationPattern extends UnparserPattern<JavaNode> {
    @Override
    public List<DisplayElement> unparse(final DisplayElement label, final DisplayElementAccessor<JavaNode> displayElementAccessor) {
        ...
        builder.space();
        builder.nodeOrNone(EXTENDS_CLASS);
        builder.space();
        builder.spaceList(IMPLEMENTS_INTERFACE); //TODO: Fix how we handle interfaces: atm I think if you have two it will show up like "implements Foo implements Bar"
        builder.newLine();
